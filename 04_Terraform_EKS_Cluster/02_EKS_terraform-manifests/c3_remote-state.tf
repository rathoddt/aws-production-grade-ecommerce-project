...
data "terraform_remote_state" "vpc" {
  ...
  config = {
    bucket = "tfstate-dev-us-east-1-5ceomn"     # Name of the remote S3 bucket where the VPC state is stored
    key    = "vpc/dev/retail-store/terraform.tfstate"        # Path to the VPC tfstate file within the bucket
